...
def main():
    ...
    raw_url = "https://web.archive.org/web/20150813114712fw_/http://vo-para.birdzberth.com/circlelist_vpk_2.html"
    ...
    for row in table_rows:
        ...
        if len(col_tags) < 4:
            continue
        if "企業名" in c0text or "サークル名" in c0text:
            continue

        circle_name = sanitize_string(c0text)
        pen_name = sanitize_string(col_tags[1].get_text(strip=True))
        circle_url = sanitize_string(col_tags[2].get_text(strip=True)) if col_tags[2] else None
        position = sanitize_string(col_tags[3].get_text(strip=True))
